...
$data modify storage aruturyuu:ore_generators positions[{data:{x:$(x),y:$(y),z:$(z),d:"$(d)"}}].data.b set from entity @e[tag=generator_setblock_dummy_$(x)_$(y)_$(z),limit=1,sort=nearest] equipment.head.id
